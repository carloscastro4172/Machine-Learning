import streamlit as st
import pandas as pd
import altair as alt
import joblib

# ==============================
# PAGE CONFIGURATION
# ==============================
st.set_page_config(page_title="EDA - Titanic", layout="wide")
st.markdown("<h1 style='text-align:center;color:#1f4e79;'> Titanic Survival - EDA</h1>", unsafe_allow_html=True)

# ==============================
# DATA UPLOAD
# ==============================
uploaded_file = st.file_uploader("Upload CSV dataset", type=["csv"])

if uploaded_file is None:
    st.info("Upload a CSV file to start the analysis.")
    st.stop()

df = pd.read_csv(uploaded_file)

st.subheader("Data preview")
n_rows = st.slider("Number of rows to display", min_value=5, max_value=len(df), value=5, step=5)
st.dataframe(df.head(n_rows))

# ==============================
# SIDEBAR: FILTERS
# ==============================
st.sidebar.header("Analysis filters")

# Categorical
sel_pclass = st.sidebar.multiselect("Passenger Class", sorted(df["Pclass"].unique()), default=sorted(df["Pclass"].unique()))
sel_sex = st.sidebar.multiselect("Sex", sorted(df["Sex"].unique()), default=sorted(df["Sex"].unique()))
sel_embarked = st.sidebar.multiselect("Port of Embarkation", sorted(df["Embarked"].dropna().unique()), default=sorted(df["Embarked"].dropna().unique()))
only_survived = st.sidebar.checkbox("Show only survivors", value=False)

# Numerical
age_range = st.sidebar.slider("Age", float(df["Age"].min()), float(df["Age"].max()), (float(df["Age"].min()), float(df["Age"].max())))
fare_range = st.sidebar.slider("Fare", float(df["Fare"].min()), float(df["Fare"].max()), (float(df["Fare"].min()), float(df["Fare"].max())))

# Apply filters
df_f = df.copy()
df_f = df_f[df_f["Pclass"].isin(sel_pclass)]
df_f = df_f[df_f["Sex"].isin(sel_sex)]
df_f = df_f[df_f["Embarked"].isin(sel_embarked)]
df_f = df_f[df_f["Age"].between(age_range[0], age_range[1])]
df_f = df_f[df_f["Fare"].between(fare_range[0], fare_range[1])]

if only_survived:
    df_f = df_f[df_f["Survived"] == 1]

if df_f.empty:
    st.warning("No data available for the selected filters.")
    st.stop()

# ==============================
# LABEL MAP
# ==============================
label_map = {
    "Age": "Age",
    "Fare": "Fare",
    "Pclass": "Class",
    "Sex": "Sex",
    "Embarked": "Port of Embarkation",
    "Survived": "Survival",
    "SibSp": "Siblings/Spouses aboard",
    "Parch": "Parents/Children aboard"
}

# ==============================
# MAIN KPIs
# ==============================
st.markdown("## Key Metrics")
col1, col2, col3, col4 = st.columns(4)
surv_rate = df_f["Survived"].mean() * 100
avg_age = df_f["Age"].mean()
avg_fare = df_f["Fare"].mean()
total_passengers = len(df_f)

col1.metric("Survival Rate", f"{surv_rate:.2f}%")
col2.metric("Average Age", f"{avg_age:.1f} years")
col3.metric("Average Fare", f"${avg_fare:.2f}")
col4.metric("Total Passengers", f"{total_passengers:,}")

st.markdown("---")

# ==============================
# TABS
# ==============================
tab1, tab2, tab3, tab4, tab5 = st.tabs(["Distributions", "Comparisons", "Correlations", "Insights", "Prediction"])

# ---- Tab 1: Distributions
with tab1:
    st.subheader("Age and Fare Distribution")
    for col in ["Age", "Fare"]:
        chart = alt.Chart(df_f).mark_bar(opacity=0.8).encode(
            alt.X(col, bin=alt.Bin(maxbins=30), title=label_map.get(col, col)),
            alt.Y('count()', title='Frequency'),
            tooltip=[col]
        ).properties(height=300)
        st.altair_chart(chart, use_container_width=True)

    st.subheader("Boxplot of Age by Category")
    x_cat = st.selectbox("Category", ["Pclass", "Sex", "Embarked"], index=0)
    chart = alt.Chart(df_f).mark_boxplot().encode(
        x=alt.X(x_cat, title=label_map.get(x_cat, x_cat)),
        y=alt.Y("Age", title="Age"),
        color=x_cat
    ).properties(height=400)
    st.altair_chart(chart, use_container_width=True)

# ---- Tab 2: Comparisons
with tab2:
    st.subheader("Survival by Category")
    for var in ["Pclass", "Sex", "Embarked"]:
        chart = alt.Chart(df_f).mark_bar().encode(
            x=alt.X(var, title=label_map.get(var, var)),
            y=alt.Y("count()", title="Frequency"),
            color=alt.Color("Survived:N", scale=alt.Scale(scheme="set1"), title="Survival"),
            tooltip=[var, "Survived"]
        ).properties(height=300)
        st.altair_chart(chart, use_container_width=True)

# ---- Tab 3: Correlations
with tab3:
    st.subheader("Correlation Matrix")
    num_vars = ["Age", "Fare", "SibSp", "Parch", "Survived"]
    corr = df_f[num_vars].corr()

    corr_long = corr.reset_index().melt("index")

    base = alt.Chart(corr_long).encode(
        x=alt.X("index:N", title="Variable"),
        y=alt.Y("variable:N", title="Variable")
    )

    rects = base.mark_rect().encode(
        color=alt.Color("value:Q", scale=alt.Scale(scheme="redblue", domain=(-1, 1)), title="Correlation")
    )

    text = base.mark_text(size=14, color="black").encode(
        text=alt.Text("value:Q", format=".0%")
    )

    corr_chart = rects + text
    corr_chart = corr_chart.properties(height=400)

    st.altair_chart(corr_chart, use_container_width=True)

# ---- Tab 4: Insights
with tab4:
    st.subheader("Key Insights")
    bullets = []
    if "Sex" in df_f.columns:
        surv_by_sex = df_f.groupby("Sex")["Survived"].mean() * 100
        bullets.append(f"- Women: {surv_by_sex.get('female', 0):.2f}% survival vs. Men: {surv_by_sex.get('male', 0):.2f}%.")
    if "Pclass" in df_f.columns:
        surv_by_class = df_f.groupby("Pclass")["Survived"].mean() * 100
        best_class = surv_by_class.idxmax()
        bullets.append(f"- Highest survival was in Class {best_class} ({surv_by_class[best_class]:.2f}%).")
    if "Age" in df_f.columns:
        young = df_f[df_f["Age"] <= 18]["Survived"].mean() * 100
        adult = df_f[df_f["Age"] > 18]["Survived"].mean() * 100
        bullets.append(f"- Young passengers (≤18) survived {young:.2f}% vs. adults {adult:.2f}%.")
    st.markdown("\n".join(bullets) if bullets else "No insights generated.")

# ---- Tab 5: Prediction
with tab5:
    st.subheader("Survival Prediction (Decision Tree)")

    @st.cache_resource
    def load_model():
        obj = joblib.load("titanic_pipeline.pkl")
        if hasattr(obj, "predict"):
            return obj
        if hasattr(obj, "best_estimator_") and hasattr(obj.best_estimator_, "predict"):
            return obj.best_estimator_
        if isinstance(obj, dict):
            for k in ["pipeline", "model", "clf", "best_estimator_"]:
                if k in obj and hasattr(obj[k], "predict"):
                    return obj[k]
        raise TypeError("The .pkl file does not contain a valid estimator with predict().")

    try:
        pipeline = load_model()
    except Exception as e:
        st.error("Could not load `titanic_pipeline.pkl`.")
        st.code(str(e))
        st.stop()

    with st.form("prediction_form"):
        pclass = st.selectbox("Passenger Class", [1, 2, 3], index=0)
        sex = st.selectbox("Sex", ["male", "female"], index=0)
        age = st.slider("Age", min_value=0, max_value=90, value=30)
        sibsp = st.slider("Siblings/Spouses (SibSp)", min_value=0, max_value=10, value=0)
        parch = st.slider("Parents/Children (Parch)", min_value=0, max_value=10, value=0)
        fare = st.slider("Fare", min_value=0.0, max_value=600.0, value=32.0, step=0.5)
        embarked = st.selectbox("Port of Embarkation", ["C", "Q", "S"], index=2)

        submitted = st.form_submit_button("Predict")

    if submitted:
        new_data = pd.DataFrame([{
            "Pclass": pclass, "Sex": sex, "Age": age,
            "SibSp": sibsp, "Parch": parch, "Fare": fare, "Embarked": embarked
        }])

        pred = pipeline.predict(new_data)[0]
        prob_txt = ""
        if hasattr(pipeline, "predict_proba"):
            prob = pipeline.predict_proba(new_data)[0][1] * 100
            prob_txt = f" (Survival probability: {prob:.2f}%)"

        if pred == 1:
            st.success("The passenger survives" + prob_txt)
        else:
            st.error("The passenger does not survive" + (f" (Non-survival probability: {100-prob:.2f}%)" if prob_txt else ""))
